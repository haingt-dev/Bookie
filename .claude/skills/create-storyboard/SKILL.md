...
Narration text (`/write-video`), timestamps (from `make voice`), and image prompts (`/generate-prompts`) belong to later pipeline steps. The storyboard is a direction document — it sets what each scene should accomplish and feel like, not the exact words, duration, or visuals.
